...
- Inside a git worktree, `cd` changes the shell's directory but not necessarily a tool's own path resolver — verify `pwd`/`git branch --show-current` before any file edit (own or a dispatched subagent's), use freshly-derived worktree-absolute paths, and independently check `git log` in both the worktree and the main checkout after any subagent claims to have committed. See `memory/lessons-learned.md#LL-0022`.
- In any migration that adds both a column and a constraint on it, order statements backfill-before-constraint, never constraint-before-backfill — an empty local dev reset will pass either order, but a real database with existing rows only survives the correct one. See `memory/lessons-learned.md#LL-0023`.
- When a function's return value is also bound to a framework's declared response/serialization contract (FastAPI `response_model`, DRF serializer, GraphQL resolver type), a test that calls the function directly does not exercise that contract — write the regression test through the real request path. See `memory/lessons-learned.md#LL-0024`.
